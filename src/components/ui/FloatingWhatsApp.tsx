...
export function FloatingWhatsApp() {
  ...
  const [phoneNumber, setPhoneNumber] = useState("6281390820723");

  useEffect(() => {
    fetch("/api/settings")
      .then((res) => res.json())
      .then((res) => {
        if (res.success && res.data?.contactWhatsapp) {
          let phone = res.data.contactWhatsapp.replace(/[^0-9]/g, "");
  ...
  return (
    <a
      ...
      onMouseOver={(e) => {
        ...
        e.currentTarget.style.backgroundColor = "#B08D3D";
